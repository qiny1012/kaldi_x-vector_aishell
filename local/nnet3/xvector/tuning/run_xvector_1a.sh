...
## 通过sid/nnet3/xvector/get_egs定义nnet的样本
## 参数num-repeats，表示训练的次数
## 如果你的num-repeats数量超过200,修改frames-pre-iter。
## min-frames-per-chunk and --max-frames-per-chunk 表示最大长度和最小长度。
## 为了确保脚本能够正确运行，需要在stage3之后添加exit 1
## 
